...
const RealMapBase = ({ places = [], isRoute = false, mapStatus, mapErrorMsg, currentCity, onMarkerClick, onMapClick, lockViewport = true, mapView, onMapViewChange, routeSegments = [], className = '', heightClassName = '' }) => {
  ...
  // 第三个useEffect：只负责更新marker和polyline
  // 修复：用 key ref 做稳定比较，只有真正数据变化时才重建
  useEffect(() => {
    const map = mapInstance.current;
    if (!map) return;
    if (isUserInteractingRef.current) return;

    const newPlacesKey = places.map(p => p.id).join(',');
    const newSegmentsKey = routeSegments.map(s => `${s?.distance || 0}_${s?.time || 0}`).join('|');
    if (placesKeyRef.current === newPlacesKey && segmentsKeyRef.current === newSegmentsKey) return;
    placesKeyRef.current = newPlacesKey;
    segmentsKeyRef.current = newSegmentsKey;
  ...
  // fitView 独立 effect：每次 places 真正变化时才调整（修复4：减级联循环）
  useEffect(() => {
    const map = mapInstance.current;
    if (!map) return;
    if (places.length === 0 || !isRoute) return;
    const m = markersRef.current;
    if (m.length === 0) return;
    map.setFitView(m, false, [60, 60, 60, 60]);
  }, [places, isRoute]);
  ...
  return (
    <div className={`w-full min-h-[300px] touch-none ${className} ${heightClassName}`.trim()} style={{ backgroundColor: COLORS.light }}>
      <div ref={containerRef} className="w-full h-full" />
    </div>
  );
};

// 修复2：自定义 memo 比较函数，深比较 places 和 routeSegments
const RealMap = memo(RealMapBase, (prev, next) => {
  if (prev.mapStatus !== next.mapStatus) return false;
  if (prev.isRoute !== next.isRoute) return false;
  if (prev.currentCity !== next.currentCity) return false;
  if (prev.className !== next.className) return false;
  if (prev.heightClassName !== next.heightClassName) return false;
  if (prev.lockViewport !== next.lockViewport) return false;
  if (prev.places.length !== next.places.length) return false;
  for (let i = 0; i < prev.places.length; i++) {
    if (prev.places[i]?.id !== next.places[i]?.id) return false;
    if (prev.places[i]?.name !== next.places[i]?.name) return false;
  }
  if (prev.routeSegments.length !== next.routeSegments.length) return false;
  return true;
});
...
export default function App() {
    ...
    if (user && !user.is_anonymous && supabase) {
      const fetchCloudData = async () => {
        ...
        try {
          ...
          if (tRes.data) {
            setTrips((prev) => {
              const merged = new Map(prev.map((trip) => [trip.id, trip]));
              let hasNew = false;
              tRes.data.forEach((trip) => {
                const normalized = normalizeTrip(trip);
                if (!merged.has(trip.id)) hasNew = true;
                merged.set(trip.id, normalized);
              });
          ...
          if (mRes.data) {
            setGlobalMemos((prev) => {
              ...
              let hasNew = false;
              mRes.data.forEach((memo) => {
                if (!merged.has(memo.id)) hasNew = true;
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ), [activeTripId, JSON.stringify(trips.map(t => ({ id: t.id, places: t.places, days: t.days, name: t.name })))]);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeTrip, currentRouteDay, JSON.stringify(savedPlaces.map(p => p.id)), segmentModes, currentCity, mapStatus, showRoutePanel]);
  ...
  // 修复5：createTrip 增加重试提示
  const createTrip = async (newTrip) => {
    const normalizedTrip = normalizeTrip(newTrip);
    setTrips(prev => [normalizedTrip, ...prev]);
    if (user && !user.is_anonymous && supabase) {
      try { await supabase.from('trips').upsert({ ...normalizedTrip, user_id: user.id }); } catch(e){ 
        logCloudError('Create trip', e);
        console.warn('行程已保存在本地，云端同步失败，稍后重试');
      }
    }
  };
  ...
  // 修复3 + 修复4：用 useMemo 稳定 currentDayTripPlaces，保留原始索引
  const currentDayTripPlaces = useMemo(() => {
    return currentDayPlaceIds
      .map((pid) => savedPlaces.find((place) => place.id === pid))
      .filter(Boolean);
  }, [currentDayPlaceIds.join(','), savedPlaces.length]);
  ...
  return (
    <div className="min-h-[100dvh] w-full flex justify-center bg-gray-100 sm:bg-[#f0f4f8]" style={{ fontFamily: '"PingFang SC","Hiragino Sans GB","Noto Sans SC","Microsoft YaHei",sans-serif' }}>
      <div className="w-full sm:max-w-[960px] h-[100dvh] flex flex-col relative bg-white overflow-hidden shadow-2xl min-h-0">
        ...
        {/* 修复1 + 修复4：地图移出 overflow-y-auto，打断级联 */}
        {/* ==================================================== */}
        {showRoutePanel && activeTripId && (
          <div className="fixed inset-0 z-[120] flex flex-col bg-slate-50 min-h-0">
            ...
            <div className="shrink-0">
              <div className="mx-auto w-full max-w-[960px] px-3 sm:px-5 pt-3 sm:pt-4 pb-0">
